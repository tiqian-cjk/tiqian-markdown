...
internal const val MARKDOWN_IMAGE_VIEWER_MAX_SCALE = 5f
internal const val MARKDOWN_IMAGE_VIEWER_TEMP_MIN_SCALE = 0.85f
internal const val MARKDOWN_IMAGE_VIEWER_TEMP_MAX_SCALE = 5.5f
internal const val MARKDOWN_IMAGE_VIEWER_DOUBLE_TAP_SCALE_MULTIPLIER = 2f

private const val LONG_IMAGE_RATIO_THRESHOLD = 3f

internal data class MarkdownImageViewerLayout(
...
internal data class MarkdownImageViewerTranslationBounds(
    val minX: Float,
    val maxX: Float,
    val minY: Float,
    val maxY: Float,
...
internal fun calculateMarkdownImageViewerLayout(
    viewportSize: IntSize,
    imageSize: IntSize?,
    maxLongImageWidthPx: Float,
): MarkdownImageViewerLayout? {
    ...
    return MarkdownImageViewerLayout(
        viewportSize = viewportSize,
        baseSize = Size(imageWidth, imageHeight),
        baseOrigin = Offset(
...
internal fun calculateMarkdownImageViewerMinimumScale(layout: MarkdownImageViewerLayout): Float =
    min(
        layout.viewportSize.width.toFloat() / layout.baseSize.width,
        layout.viewportSize.height.toFloat() / layout.baseSize.height,
    )

internal fun coerceMarkdownImageViewerGestureScale(scale: Float, minimumScale: Float): Float =
    scale.coerceIn(
        minimumScale * MARKDOWN_IMAGE_VIEWER_TEMP_MIN_SCALE,
        maxOf(MARKDOWN_IMAGE_VIEWER_TEMP_MAX_SCALE, minimumScale),
    )

internal fun coerceMarkdownImageViewerSettledScale(scale: Float, minimumScale: Float): Float =
    scale.coerceIn(minimumScale, maxOf(MARKDOWN_IMAGE_VIEWER_MAX_SCALE, minimumScale))

internal fun calculateMarkdownImageViewerScaledTranslation(
    ...
    if (currentScale <= 0f) return currentTranslation
...
internal fun calculateMarkdownImageViewerTranslationBounds(
    layout: MarkdownImageViewerLayout,
    scale: Float,
): MarkdownImageViewerTranslationBounds {
    ...
    return MarkdownImageViewerTranslationBounds(
...
internal fun clampMarkdownImageViewerTranslation(
    translation: Offset,
    layout: MarkdownImageViewerLayout,
    scale: Float,
): Offset {
    val bounds = calculateMarkdownImageViewerTranslationBounds(layout, scale)
    return Offset(
